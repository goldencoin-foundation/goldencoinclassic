...
#if defined(NDEBUG)
# error "Goldencoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdda28827cd11b3aeaecb4638800d2be85629e82d4af33f18cedbea28b5e52689");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Goldencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Goldencoin Signed Message:\n";
...
{
    // Goldencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Goldencoin: 210k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Goldencoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Goldencoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Goldencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Goldencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 0xbl;
        pchMessageStart[3] = 0xdd;
        hashGenesisBlock = uint256("0x53edd74ab6af8931399276e155c54d02235202b008356fa12e30ec0104c6fa3b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "cbsnews.com 07/28/2021 Simone Biles pulls out of Olympic all around competition.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047721582d96bfbeca75cd125e970e9f20b4cebbbd72c42642cfbb2e51ca8f5edb9122df8ea4eada3208bf283374ccf1a5622982a2a190140d0b204fdb2b97ea4c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1627441990;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085082879;

        if (fTestNet)
        {
            block.nTime    = 1627441992;
            block.nNonce   = 385565161;
        }
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xede9c4c0659509675ad8caed4f0e95e17d59f22533fbdca5ab52f25f6ed4db70"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x10, 0xb0, 0x0l, 0xbd }; // Goldencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoldencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoldencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoldencoinMiner(CWallet *pwallet)
{
    printf("GoldencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goldencoin-miner");
    ...
    try { loop {
        ...
        printf("Running GoldencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoldencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoldencoinMiner, pwallet));
}
